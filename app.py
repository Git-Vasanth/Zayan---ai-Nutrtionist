...
class ChatRequest(BaseModel):
    user_token: int  # ✅ ADDED: Token to fetch user from DB
    message: str
    data_level: int = 1
    plan_generated: bool = False
    nutrition_feedback: str = ""
...
from agents import Mediterranean_Nutrition_Agent
from langchain_core.messages import HumanMessage, SystemMessage, AIMessage

# --- CONVERSION UTILITY ---
def convert_db_messages_to_langchain(db_messages: List[Message]) -> List:
...
async def chat_with_ai(request: ChatRequest, db: Session = Depends(get_db)): 
    try:
        # ✅ 1. GET USER DATA FROM DB (not from frontend)
        user = db.query(UserProfile).filter(UserProfile.token == request.user_token).first()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        
        print(f"🔍 User found: {user.name}, Diet: {user.diet_type}")
        
        # ✅ 2. GET CONVERSATION HISTORY FROM DB (last 10 messages)
        db_messages = db.query(Message).filter(
            Message.user_id == user.id,
            Message.chat_type == 'ai_chat'
        ).order_by(Message.created_at.desc()).limit(5).all()
        
        # Reverse to get chronological order
        db_messages.reverse()
        
        print(f"📝 Found {len(db_messages)} messages from DB for user {user.id}")
        
        # ✅ 3. BUILD STATE WITH FRESH DB DATA
        state = {
            "messages": convert_db_messages_to_langchain(db_messages) + [HumanMessage(content=request.message)],
            "diet_type": user.diet_type,  # From DB
            "user_data": {
                "name": user.name,
                "height_cm": user.height_cm,
                "weight_kg": user.weight_kg,
        ...
        print("🚀 Invoking multi-agent system with DB data...")
        ...
        # ✅ 5. CHECK IF PLAN WAS GENERATED
        plan_created = final_state.get("db_plan_saved", False)

        return {
            "response": last_ai_message,
            "updated_level": final_state.get("data_level", 1),  # ✅ Return updated level
